...
import ParkingSlotService from '../modules/services/parkingSlot.service'; // Importuj serwis

class CarsController implements Controller {
    public path = '/api/cars';
    public router = Router();
    private carsService = new CarsService();
    private parkingSlotService = new ParkingSlotService(); // Użycie ParkingSlotService
    ...
    private createCar = async (req: Request, res: Response, next: NextFunction) => {
        ...
        // Sprawdź, czy wymagane pola zostały podane
        ...
        try {
            // Tworzenie danych samochodu bez carId
            const carData: Omit<ICars, 'carId'> = { type, model, registration };

            // Wywołanie metody serwisu do zapisania samochodu z automatycznym carId
            const car = await this.carsService.createCar(carData);

            // Aktualizacja miejsca parkingowego jako zajętego
