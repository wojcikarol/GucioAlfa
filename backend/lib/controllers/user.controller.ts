...
class UserController implements Controller {
    ...
    private initializeRoutes() {
        ...
        this.router.patch(`${this.path}/status/:userId`, checkUserIdParam, this.toggleUserStatus);
    }
    ...
    private toggleUserStatus = async (req: Request, res: Response) => {
        ...
        try {
            const user = await this.userService.updateUserStatus(userId, active, { lastUpdated: new Date() });
            if (!user) {
                return res.status(404).json({ error: "User not found" });
            }
